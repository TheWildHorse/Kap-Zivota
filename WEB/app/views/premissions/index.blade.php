@extends('layouts.scaffold')

@section('main')

<h1>All Premissions</h1>

<p>{{ link_to_route('premissions.create', 'Add New Premission', null, array('class' => 'btn btn-lg btn-success')) }}</p>

@if ($premissions->count())
	<table class="table table-striped">
		<thead>
			<tr>
				<th>Name</th>
				<th>&nbsp;</th>
			</tr>
		</thead>

		<tbody>
			@foreach ($premissions as $premission)
				<tr>
					<td>{{{ $premission->name }}}</td>
                    <td>
                        {{ Form::open(array('style' => 'display: inline-block;', 'method' => 'DELETE', 'route' => array('premissions.destroy', $premission->id))) }}
                            {{ Form::submit('Delete', array('class' => 'btn btn-danger')) }}
                        {{ Form::close() }}
                        {{ link_to_route('premissions.edit', 'Edit', array($premission->id), array('class' => 'btn btn-info')) }}
                    </td>
				</tr>
			@endforeach
		</tbody>
	</table>
@else
	There are no premissions
@endif

@stop
